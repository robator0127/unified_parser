...
class CellGroup(object):
    def __init__(self, cells: List[Optional[T_bbox]]): #这个函数有更改
        def filter_none(element):
            return element[0]!=None
        self.cells = cells
        self.bbox = (
            min(map(itemgetter(0), filter(filter_none, cells))),
            min(map(itemgetter(1), filter(filter_none, cells))),
            max(map(itemgetter(2), filter(filter_none, cells))),
            max(map(itemgetter(3), filter(filter_none, cells))),
        )
...
class Table(object):
    ...
    @property
    def rows(self) -> List[Row]: #这个函数有更改
        ...
        for y, row_cells in itertools.groupby(_sorted, itemgetter(1)):
            ...
            for index,x in enumerate(xs):
                if xdict.get(x)==None:#判断被合并的类型,是左边的单元格还是上一行
                    if index==0:#
                        result.append([None,"up"])
                    else:
                        point=(x+1,y+1) #待检测的点，横坐标是x,纵坐标是y,判断这个的位置区域
                        find_result=0
                        for j in range(index):#先判断左侧
                            if xdict.get(xs[j])==None:
                                continue
                            bbox=xdict.get(xs[j])
                            if bbox[0]<=point[0]<=bbox[2] and bbox[1]<=point[1]<=bbox[3]:
                                result.append([None,"left"]) #已找到
                                find_result=1
                        #向上找
                        for j ,temp in enumerate(privious):
                            if x in temp:#这个键值在temp里，则查找
                                bbox=temp[x]
                                if bbox[0]<=point[0]<=bbox[2] and bbox[1]<=point[1]<=bbox[3]:
                                    result.append([None,"up"])
                                    find_result=1
                    ...
                    result.append(xdict.get(x))
                
            row = Row(result)
            rows.append(row)
            privious.append(xdict) #保存已经处理过的数据
            
        return rows

    def extract(self, **kwargs: Any) -> List[List[Optional[str]]]: #这个函数有更改
        ...
        for i,row in enumerate(self.rows):
            ...
            for j,cell in enumerate(row.cells):
                if None in cell:#cell is None
                    if cell[1]=="left":
                        cell_text=arr[max(0,j-1)]
                    elif cell[1]=="up":
                        cell_text=table_arr[max(0,i-1)][j]
                    else:
                        cell_text=cell[1]
                else:
                    cell_chars = [
